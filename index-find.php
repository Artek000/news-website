...
<html>
 <?php require("header.php"); ?>
	<div class="content" style="background: #bdc3c7;">
		...
		<?php
            	$data = $_POST;
        	if (isset($data['n_find']) && ($data['n_f'] !== ''))
        	{
        	    $zapr = trim($data['n_f']);
        		$f_query = mysqli_query($dbh,"SELECT * FROM `news` WHERE title LIKE '%".$zapr."%'");
        		while($Row_finded = mysqli_fetch_assoc($f_query)){
        		   $f_result .= '<div class="allNews"><a href="../news/news_'.$Row_finded['id'].'.php"><p id="title_n">'.$Row_finded['title'].'</p><img src="'.$Row_finded['img'].'" id="news_img" style="margin: 0px;"><p id="text_n">'.$Row_finded['pre_text'].'</p></a><p id="title_date">'.$Row_finded['date'].'</p></div>';
        		}
        	}else{
